...
#page(columns: 1)[
#set text(size: 5.5pt)
#align(center, text(weight: "bold", size: 8pt)[Statistical Tests — Comprehensive Comparison])
#table(
  columns: (0.8in, 0.55in, 0.85in, 1.1in, 0.9in, 1.0in, 0.75in, 1.5in),
  table.header(
    [*Test Name*], [*Parametric or Not*], [*Decision Criteria*], [*When it is used*], [*When it is not used*], [*Places of confusion*], [*Related to which test*], [*Examples from the reading material*],
    repeat: true,
  ),
  [One Sample t-test],
  [Parametric],
  [$t = (overline(X) - mu_0) / "SE"$, df = $n - 1$],
  [To test a hypothesis about one mean in a single population where data is normally distributed (interval/ratio scale).],
  [When data is not normally distributed or is non-metric (nominal/ordinal).],
  [Deciding whether to use a t-test or Z-test depending on the situation (e.g., sample size).],
  [One Sample Z-test],
  [Testing whether the familiarity with the internet is high (>4) or not, where familiarity is measured on an interval scale of 1–7.],

  [Two Independent Sample t-test],
  [Parametric],
  [t-statistic, F-test (Levene's) for equality of variances],
  [To test whether the mean of a variable for two independent groups is different or the same.],
  [When samples are related/paired, or when data is not normally distributed.],
  [Whether to assume equal variances or not; requires testing for sample variance equality first.],
  [Mann-Whitney U test (non-parametric equivalent)],
  [Testing whether the mean familiarity with the internet is different or the same for males versus females.],

  [Paired Sample t-test],
  [Parametric],
  [t-statistic based on mean and variance of paired differences ($D$), df = $n - 1$],
  [When data for two samples relate to the same group of respondents (e.g., measuring attitude toward two different things by the same sample).],
  [When samples are drawn randomly from different populations (independent).],
  [Confusing independent vs. paired samples.],
  [Wilcoxon matched-pairs signed-ranks test, Sign test],
  [Testing whether the mean of attitude towards the internet and attitude towards technology is the same or not within a single sample.],

  [Kolmogorov-Smirnov (K-S) One-Sample Test],
  [Non-parametric],
  [K-S Z, maximum absolute difference between observed and theoretical distributions],
  [To test whether observations for a single variable could reasonably have come from a particular distribution (e.g., Normal).],
  [When you are comparing two independent samples (uses the 2-sample K-S test instead).],
  [Applying Lilliefors significance correction.],
  [Chi-square goodness-of-fit],
  [Testing the hypothesis that Internet Usage (Hrs/Week) is normally distributed.],

  [Chi-Square Goodness-of-Fit],
  [Non-parametric],
  [$chi^2 = sum (f_o - f_e)^2 / f_e$],
  [On a single categorical variable to determine if observed frequencies are similar to expected frequencies.],
  [When expected cell frequencies are too small (e.g., minimum expected frequency < 5).],
  [Ensuring the sum of percentages for the expected distribution equals exactly 100% (requires rescaling if not).],
  [K-S test, Binomial test],
  [Testing if the ratings of familiarity with the internet are equally distributed, or testing if the ratings follow a specific expected distribution like 10%, 20%, 20%, 10%, 25%, 15%.],

  [Binomial Test],
  [Non-parametric],
  [Exact Sig. (comparing observed proportion to test proportion, e.g., $p = 0.5$)],
  [As a goodness-of-fit test specifically for dichotomous (two-category) variables.],
  [When the variable has more than two categories.],
  [Only applicable to variables that can be split into exactly two groups (e.g., Yes/No).],
  [Chi-square goodness-of-fit],
  [Testing if the proportion of people utilizing "Internet Shopping" (Yes/No) or "Internet Banking" (Yes/No) is significantly different from a test proportion of 0.50.],

  [Runs Test],
  [Non-parametric],
  [Number of runs, Z-statistic],
  [To determine whether the order or sequence in which observations are obtained is random.],
  [When the actual shape of the distribution is being tested rather than the sequence of elements.],
  [Determining what constitutes a 'run' (a maximal non-empty segment of adjacent equal elements).],
  [—],
  [Evaluating a sequence of positive and negative elements (e.g., "++++ +++--++++++ -"), or testing whether the observations for familiarity with the internet were generated randomly.],

  [Chi-Square Test for Independence],
  [Non-parametric],
  [$chi^2 = sum (f_o - f_e)^2 / f_e$, df = $(r - 1) times (c - 1)$],
  [To determine whether frequencies observed at the combination of levels of two categorical variables are associated or independent.],
  [When expected counts in cells are less than 5.],
  [Maximum value of contingency coefficient depends on table size, so it should only be used to compare tables of the same size.],
  [Phi coefficient, Contingency coefficient, Cramer's V],
  [Testing if the proportion of respondents using the internet for shopping (light vs. heavy usage) is indifferent to gender (males vs. females).],

  [Mann-Whitney U Test],
  [Non-parametric],
  [U statistic, Wilcoxon W, Z-statistic],
  [To compare difference in location of two populations using two independent samples, measured on an ordinal scale.],
  [When data is paired/related, or when testing more than two groups.],
  [It uses rank combinations instead of absolute values (cases are combined and ranked).],
  [Two Independent Sample t-test, Kruskal-Wallis],
  [Testing the hypothesis that two populations (male and female) are identical with respect to their mean rank of familiarity with the internet.],

  [Wilcoxon Matched-Pairs Signed-Ranks Test],
  [Non-parametric],
  [Z-statistic based on positive/negative ranks],
  [Analyzes differences between paired observations, taking into account the magnitude of the differences.],
  [When samples are independent.],
  [Handling ties (differences of zero) in the ranking process.],
  [Paired Sample t-test, Sign test],
  [Testing whether the mean rank of attitude towards the internet is the same as the mean rank of attitude towards technology.],

  [Paired Sample Sign Test],
  [Non-parametric],
  [Exact Sig. based on binomial distribution],
  [Analyzes differences between paired observations, taking into account only the sign (pluses vs. minuses) of the differences.],
  [When the magnitude of the differences is available and meaningful (Wilcoxon is better).],
  [It is less powerful than Wilcoxon because it ignores the magnitude of the difference.],
  [Wilcoxon matched-pairs signed-ranks test],
  [Analyzing the frequencies of positive differences, negative differences, and ties between the paired variables "Attitude toward Technology" and "Attitude toward Internet".],

  [One-way ANOVA],
  [Parametric],
  [$F = "MS"_x / "MS"_"error"$, df = $(c - 1)$ and $(N - c)$],
  [Test of means for two or more populations with one categorical independent variable and a metric dependent variable.],
  [When error terms are correlated, variances are not constant, or distribution is not normal.],
  [Rejecting the null hypothesis only tells you that not all means are equal; you must use multiple comparison tests (like LSD) to find which specific means differ.],
  [Kruskal-Wallis test (non-parametric equivalent)],
  [A department store determining the effect of different levels of in-store promotion (High, Medium, Low) on store sales.],

  [n-way (Factorial) ANOVA],
  [Parametric],
  [F-statistics for main effects and interaction effects],
  [To measure the effects of two or more independent categorical variables at various levels on a metric dependent variable.],
  [—],
  [Interpreting interaction effects (where the effect of one variable depends on the level of another).],
  [One-way ANOVA],
  [Measuring the combined effect of two independent variables: the amount of humor (No, Medium, High) and the amount of store information (Low, Medium, High). Another example tests the effect of in-store promotion and couponing on sales.],

  [Analysis of Covariance (ANCOVA)],
  [Parametric],
  [F-statistics],
  [To account for the influence of uncontrolled independent variables (covariates) while examining differences in mean values related to controlled independent variables.],
  [—],
  [Differentiating the impact of the categorical factors from the continuous covariates.],
  [ANOVA],
  [Determining how different price levels affect cereal consumption while controlling for the covariate of household size. Another example determines the effect of in-store promotion and couponing on sales while controlling for the clientele effect.],

  [Kruskal-Wallis Test],
  [Non-parametric],
  [H statistic (Chi-Square distribution)],
  [Extension of Mann-Whitney test. Examines difference in medians across more than two groups (one-way ANOVA on ranks).],
  [When parametric assumptions are fully met (ANOVA is more powerful).],
  [Cases from all k groups are ordered into a single ranking to calculate rank sums.],
  [One-way ANOVA, Mann-Whitney U],
  [Examining differences in sales by ordering and comparing the mean ranks across three different levels of promotion.],

  [k-Sample Median Test],
  [Non-parametric],
  [Pearson's Chi-Square based on median split],
  [Tests null hypothesis that medians of populations from 2 or more samples are identical by grouping data into "> Median" and "≤ Median".],
  [When there are few tied rankings (Kruskal-Wallis is more powerful).],
  [It is less powerful than Kruskal-Wallis because it ignores exact rank and only uses location relative to the overall median.],
  [Kruskal-Wallis Test],
  [Grouping observed sales data into "> Median" or "≤ Median" frequencies and testing for differences across three levels of promotion.],
)
]
